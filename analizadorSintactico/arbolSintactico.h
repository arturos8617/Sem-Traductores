...
public:
	...
	char dimeTipo() { // A�adimos el m�todo dimeTipo para obtener el tipo de dato en forma de char
		if (tipoDato.compare("int") == 0) return 'i';
		if (tipoDato.compare("float") == 0) return 'f';
		if (tipoDato.compare("string") == 0) return 's';
		if (tipoDato.compare("void") == 0) return 'v';
		return ' '; // En caso de que no se encuentre un tipo v�lido
	}
